...
console.log("Stok Kodlu ve Birimli yeni Excel dosyaları üretiliyor. Lütfen bekleyin...");

// ==========================================
// 1. KAOS DEPO EXCELİ (Reklamın Başlangıcı İçin)
// ==========================================
const kaosData = [];
// Kasten iğrenç başlıklar ama eşleştirmeye uygun veriler eklendi:
kaosData.push([
    'KOD_(BARKOD)', // Eşleştirme: Stok Kodu
    'MALZEME_TANIMI_(SİLMEYİN!)', // Eşleştirme: Ürün Adı
    'GÜNCEL ADET (SAYIM BEKLİYOR!!)', // Eşleştirme: Miktar
    'BİRİM_CİNSİ', // Eşleştirme: Birim
    'FİYAT_$_EU_TL_KARISIK', // Eşleştirme: Fiyat
    'LOKASYON_DEPO_RAF', 
    'AHMET USTANIN NOTLARI'
]);

const birimler = ['Adet', 'Kg', 'Metre', 'Litre', 'Paket'];

for (let i = 1; i <= 8000; i++) {
    const isError = Math.random() > 0.85; // %15 ihtimalle bozuk veri üret
    
    // Rastgele birim seçimi veya hata
    const rastgeleBirim = isError ? '???' : birimler[Math.floor(Math.random() * birimler.length)];
    
    // Rastgele stok kodu veya hata
    const stokKodu = isError ? `KOD-YOK-${Math.floor(Math.random() * 1000)}` : `STK-2026-${10000 + i}`;

    kaosData.push([
        stokKodu,
        isError ? `Bilinmeyen Parça / BULUNAMADI - ${Math.floor(Math.random() * 1000)}` : `Çelik_Profil_${i}_mm_X_Kalite_Fason_Giden`,
...
ws1['!cols'] = [{ wch: 20 }, { wch: 40 }, { wch: 15 }, { wch: 10 }, { wch: 20 }, { wch: 15 }, { wch: 50 }];
...
console.log("Dosyalar başarıyla oluşturuldu! Videoda şov zamanı!");
